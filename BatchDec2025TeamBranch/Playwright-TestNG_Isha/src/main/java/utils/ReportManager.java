 ...
 *
 * Responsibilities: - Initialize ExtentReports (once per run) - Manage
 * thread-safe ExtentTest instances - Log PASS / FAIL / SKIP / INFO steps -
 * Capture screenshots via ScreenshotUtil - Flush reports at the end of
 * execution
 *
 * Design principles: - Thread-safe via ThreadLocal - Screenshot responsibility
 * owned here - Tests and PageActions never handle screenshots directly
 * =====================================================================
 */
public final class ReportManager {
	...
	/**
	 * Initializes ExtentReports. Safe to call multiple times (idempotent).
	 */
	public static synchronized void initReports() {
		...
		reportDir = String.join(File.separator, baseDir, year, month, date, "run_" + time);

		new File(reportDir).mkdirs();

		ExtentSparkReporter spark = new ExtentSparkReporter(reportDir + File.separator + "TestReport.html");
		...
		extent.setSystemInfo("Environment", ConfigManager.getProperty("env", "QA"));
	}
	...
	/**
	 * Flushes ExtentReports to disk. Should be called once after all tests.
	 */
	...
	/**
	 * Logs a PASS step. Screenshot capture is controlled via config:
	...
	public static void stepPass(String message, Page page) {
		...
		boolean captureOnPass = ConfigManager.getBoolean("screenshot.on.pass");
		...
		if (captureOnPass && page != null) {
			...
			if (base64 != null) {
				t.pass(message, MediaEntityBuilder.createScreenCaptureFromBase64String(base64).build());
				return;
			}
		}
	...
	public static void stepFail(String message, Page page) {
		...
		String base64 = (page != null) ? ScreenshotUtil.captureBase64(page) : null;

		if (base64 != null) {
			t.fail(message, MediaEntityBuilder.createScreenCaptureFromBase64String(base64).build());
		} else {
			t.fail(message);
		}
	}
